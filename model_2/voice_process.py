...
#对于date信息，voice处理方法和msg完全一致
#1. 对每个特征都分in out 两类进行处理
#2. 对用户按照每日的统计量进行特征提取
#3. 分别用bag 和直接one-hot处理date
...
#对通话的type进行分类， 同样也是区分in out 进行处理
...
#以下特征均区分 in out， 并且都将in out 的差异diff也作为特征
...
#对tel_opp_len 进行分类， 与短信表的处理方式相同 分裂点选择为5 ，11 
