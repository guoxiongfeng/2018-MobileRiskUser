...
def get_other_feature(wa_data):
    ...
    wa_data['visit_per_dura'] = wa_data['visit_dura'] / wa_data['visit_cnt'] #平均访问时长
    wa_data['flow_amount'] = wa_data['up_flow'] + wa_data['down_flow'] #总流量

    wa_visit_per_dura_static = wa_data.groupby(['uid', 'wa_date'])['visit_per_dura'].sum().groupby(['uid']).agg(
    ['sum', 'std', 'max', 'mean', 'median']).add_prefix('wa_visit_per_dura_') #按天统计的访问时间统计量

    #上下行速度计算， 为了防止nan， 分子分母+1处理
